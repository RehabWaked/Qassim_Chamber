...
import Pages.MinistryOfIntriorAffairsPage;
import Pages.RequestsLogPage;
import data.MinistryOfIntriorAffairs_JSONDataReader;
public class MinistryOfIntriorAffairsTest extends TestBase{

	RequestsLogPage ReqLogObj;
	boolean skipTest;	
	MinistryOfIntriorAffairsPage IntriorAffairsOBJ;
	MinistryOfIntriorAffairs_JSONDataReader IntriorAffairsReader;
	...
	public void CheckUserAvailableBalance() throws InterruptedException, FileNotFoundException, IOException, ParseException
	{
		...
		IntriorAffairsReader = new MinistryOfIntriorAffairs_JSONDataReader();
		IntriorAffairsReader.jsonReader_FillData();
		RequestCost = GetRequestCost(IntriorAffairsReader.RequestName);
	...
	public void FillDataAndCreateRequest() throws InterruptedException, FileNotFoundException, IOException, ParseException {
		...
		IntriorAffairsOBJ= new MinistryOfIntriorAffairsPage(driver);
		IntriorAffairsReader = new MinistryOfIntriorAffairs_JSONDataReader();
		IntriorAffairsReader.jsonReader_FillData();
		OpenNewRequest( IntriorAffairsReader.RequestName);
		IntriorAffairsOBJ.createNewRequest_MinistryOfIntriorAffairsRequest(IntriorAffairsReader.ApplicantName, IntriorAffairsReader.RequestNumber ,IntriorAffairsReader.ApplicantIDNumber);
	}
	...
	{
		OpenTheRelatedCard_CheckCreatedNewRrequest();
		IntriorAffairsOBJ.UserCanceledNewRequestCreated(IntriorAffairsReader.CancelText);
		CheckBalanceAfterSubmission_Cancellation_Rejection();
		ReqLogObj.ViewRequestAttachment();
		ReqLogObj.ViewCancelReason(IntriorAffairsReader.CancelText);
	}
	...
	void CreateRequestWithoutData() throws FileNotFoundException, IOException, ParseException, InterruptedException
	{
		...
		if (HasBalance) {
			...
			IntriorAffairsOBJ= new MinistryOfIntriorAffairsPage(driver);
			OpenNewRequest( IntriorAffairsReader.RequestName);
			IntriorAffairsOBJ.createNewRequest_MinistryOfIntriorAffairsRequestWithoutData();
			skipTest = false;
		}
		else {
			System.out.println("You Don't have available Balance to Submit New Requests");
