...
import Pages.ExtendVisaFromPassportsRequestRequest;
import Pages.RequestsLogPage;
import data.ExtendVisaFromPassportsRequest_JSONDataReader;

public class ExtendVisaFromPassportsRequestTest extends TestBase{
	
	RequestsLogPage ReqLogObj;
	boolean skipTest;	
	ExtendVisaFromPassportsRequestRequest ExtendFromPassportsOBJ;
	ExtendVisaFromPassportsRequest_JSONDataReader ExtendFromPassportsReader;
	...
	public void CheckUserAvailableBalance() throws InterruptedException, FileNotFoundException, IOException, ParseException
	{
		...
		ExtendFromPassportsReader = new ExtendVisaFromPassportsRequest_JSONDataReader();
		ExtendFromPassportsReader.jsonReader_FillData();
		RequestCost = GetRequestCost(ExtendFromPassportsReader.RequestName);
	...
	public void FillDataAndCreateRequest() throws InterruptedException, FileNotFoundException, IOException, ParseException {
		...
		ExtendFromPassportsOBJ = new ExtendVisaFromPassportsRequestRequest(driver);
		ExtendFromPassportsReader = new ExtendVisaFromPassportsRequest_JSONDataReader();
		ExtendFromPassportsReader.jsonReader_FillData();
		OpenNewRequest(ExtendFromPassportsReader.RequestName);
		ExtendFromPassportsOBJ.createNewRequest_ExtendVisaFromPassportsRequestRequest(ExtendFromPassportsReader.WorkRegion, ExtendFromPassportsReader.IdNumber, ExtendFromPassportsReader.SirName, ExtendFromPassportsReader.Nationality ,ExtendFromPassportsReader.IdSource, ExtendFromPassportsReader.JobName, ExtendFromPassportsReader.VisaNo);
	}
	...
	{
		OpenTheRelatedCard_CheckCreatedNewRrequest();
		ExtendFromPassportsOBJ.UserCanceledNewRequestCreated(ExtendFromPassportsReader.CancelText);
		CheckBalanceAfterSubmission_Cancellation_Rejection();
		ReqLogObj.ViewRequestAttachment();
		ReqLogObj.ViewCancelReason(ExtendFromPassportsReader.CancelText);
	}
	...
	void CreateRequestWithoutData() throws FileNotFoundException, IOException, ParseException, InterruptedException
	{
		...
		if (HasBalance) {
			...
			ExtendFromPassportsOBJ = new ExtendVisaFromPassportsRequestRequest(driver);
			OpenNewRequest(ExtendFromPassportsReader.RequestName);
			ExtendFromPassportsOBJ.createNewRequest_ExtendVisaFromPassportsRequestRequestWithoutData();
			skipTest = false;
		}
		else {
			System.out.println("You Don't have available Balance to Submit New Requests");
