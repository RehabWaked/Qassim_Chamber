...
import Pages.AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestPage;
import Pages.RequestsLogPage;
import data.AuthorizationExtractingSubCommercialRegisterForSoleProprietorshipCompany_JSONDataReader;

public class AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestTest extends TestBase{

	RequestsLogPage ReqLogObj;
	boolean skipTest;
	AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestPage AuthorizeExtractSoleCompanyOBJ;
	AuthorizationExtractingSubCommercialRegisterForSoleProprietorshipCompany_JSONDataReader AuthorizeExtractSoleCompanyReader;
	...
	public void CheckUserAvailableBalance() throws InterruptedException, FileNotFoundException, IOException, ParseException
	{
		...
		AuthorizeExtractSoleCompanyReader = new AuthorizationExtractingSubCommercialRegisterForSoleProprietorshipCompany_JSONDataReader();
		AuthorizeExtractSoleCompanyReader.jsonReader_FillData();
		RequestCost = GetRequestCost(AuthorizeExtractSoleCompanyReader.RequestName);
	...
	public void FillDataAndCreateRequest() throws InterruptedException, FileNotFoundException, IOException, ParseException {
		...
		AuthorizeExtractSoleCompanyOBJ= new AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestPage(driver);
		AuthorizeExtractSoleCompanyReader = new AuthorizationExtractingSubCommercialRegisterForSoleProprietorshipCompany_JSONDataReader();
		AuthorizeExtractSoleCompanyReader.jsonReader_FillData();
		OpenNewRequest(AuthorizeExtractSoleCompanyReader.RequestName);
		AuthorizeExtractSoleCompanyOBJ.createNewRequest_AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequest(AuthorizeExtractSoleCompanyReader.WorkRegion, AuthorizeExtractSoleCompanyReader.OfficeName, AuthorizeExtractSoleCompanyReader.OfficeServiceNumber, AuthorizeExtractSoleCompanyReader.OTP);;
	}
	...
	{
		OpenTheRelatedCard_CheckCreatedNewRrequest();
		AuthorizeExtractSoleCompanyOBJ.UserCanceledNewRequestCreated(AuthorizeExtractSoleCompanyReader.CancelText);
		CheckBalanceAfterSubmission_Cancellation_Rejection();
		ReqLogObj.ViewRequestAttachment();
		ReqLogObj.ViewCancelReason(AuthorizeExtractSoleCompanyReader.CancelText);
	}
	...
	void CreateRequestWithoutData() throws FileNotFoundException, IOException, ParseException, InterruptedException
	{
		...
		if (HasBalance) {
			...
			AuthorizeExtractSoleCompanyOBJ= new AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestPage(driver);
			OpenNewRequest(AuthorizeExtractSoleCompanyReader.RequestName);
			AuthorizeExtractSoleCompanyOBJ.createNewRequest_AuthorizationToExtractSub_CommercialRegisterForSoleProprietorshipCompanyRequestWithoutData();
			skipTest = false;
		}
		else {
			System.out.println("You Don't have available Balance to Submit New Requests");
