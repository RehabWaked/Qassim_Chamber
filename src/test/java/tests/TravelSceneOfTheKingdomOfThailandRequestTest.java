...
import Pages.TravelSceneOfTheKingdomOfThailandRequestPage;
import data.TravelSceneOfTheKingdomOfThailand_JSONDataReader;

public class TravelSceneOfTheKingdomOfThailandRequestTest extends TestBase{

	
	RequestsLogPage ReqLogObj;
	boolean skipTest;
	TravelSceneOfTheKingdomOfThailandRequestPage TravelSceneOBJ;
	TravelSceneOfTheKingdomOfThailand_JSONDataReader TravelSceneReader;
	...
	public void CheckUserAvailableBalance() throws InterruptedException, FileNotFoundException, IOException, ParseException
	{
		...
		TravelSceneReader = new TravelSceneOfTheKingdomOfThailand_JSONDataReader();
		TravelSceneReader.jsonReader_FillData();
		RequestCost = GetRequestCost(TravelSceneReader.RequestName);
	...
	public void FillDataAndCreateRequest() throws InterruptedException, FileNotFoundException, IOException, ParseException {
		...
		TravelSceneOBJ = new TravelSceneOfTheKingdomOfThailandRequestPage(driver);
		TravelSceneReader = new TravelSceneOfTheKingdomOfThailand_JSONDataReader();
		TravelSceneReader.jsonReader_FillData();
		OpenNewRequest( TravelSceneReader.RequestName);
		TravelSceneOBJ.createNewRequest_TravelSceneOfTheKingdomOfThailandRequest(TravelSceneReader.SirName, TravelSceneReader.Passport, TravelSceneReader.PassportSource,TravelSceneReader.OTP);
	}
	...
	void CancelRequestCreatedTest() throws FileNotFoundException, IOException, ParseException, InterruptedException
	{
		OpenTheRelatedCard_CheckCreatedNewRrequest();
		CheckBalanceAfterSubmission_Cancellation_Rejection();
		TravelSceneOBJ.UserCanceledNewRequestCreated(TravelSceneReader.CancelText);
		CheckBalanceAfterSubmission_Cancellation_Rejection();
		ReqLogObj.ViewRequestAttachment();
		ReqLogObj.ViewCancelReason(TravelSceneReader.CancelText);
	}
	...
	void CreateRequestWithoutData() throws FileNotFoundException, IOException, ParseException, InterruptedException
	{
		...
		if (HasBalance) {
			...
			TravelSceneOBJ = new TravelSceneOfTheKingdomOfThailandRequestPage(driver);
			OpenNewRequest( TravelSceneReader.RequestName);
			TravelSceneOBJ.createNewRequest_TravelSceneOfTheKingdomOfThailandRequestWithoutData();
			skipTest = false;
		}
		else {
			System.out.println("You Don't have available Balance to Submit New Requests");
